...
class BotrunnerWebserviceController(BaseXMLRPCController):
   ...
   # needs to pass as string, since xmlrpc doesn't support long :-/
    ...
    # request info on an ai that can be downloaded that matches a match
    ...
    # - otherwise it checks with each host if there is an ai that it would be
    # useful to download
    # - note that where there are multiple sessions for a botrunner, only one
    # session will be
    #   assigned a specific ai to download, ie they can download different
    # ones in paralle
    #   but not the same one.  That's for configurations like aegis' where all
    # AIs are in the same
    #   physical directory across all instances
    # Note that after downloading, the botrunner is expected to reregister
    # capabilities before
    # doing anything else, eg before making new requests for download
    # Also, we'll just assume that any botrunner that calls this is ok with
    # downloadnig stuff
    ...
    def getdownloadrequest(self, botrunnername, sharedsecret, sessionid):
        ...
        # this should not be called until the ai has been registered,
